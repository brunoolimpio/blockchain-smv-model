...
if autoGen == "no":
    ...
    numUsers = random.randint(2,100)
    for i in range(1,numUsers+1):
        balance = random.randint(0,25)
        users.append("user{} : createUser(self,{});".format(i, balance))
...
for k in range(0,len(transactions)):
    print("   " + transactions[k])
print("")    
for s in range(0,len(blocks)):
    print("   " + blocks[s])
print("")
print("ASSIGN")
for i in range(1,int((len(users)/2)+1)):
    print("   init(user{}.busy) := TRUE;".format(i))
print("")
print("DEFINE")

for i in range(1,len(users)+1):
    userTxn = userTxns["user{}".format(i)]
    string_update = "   user{}.update := ".format(i)
    if len(userTxn) != 0:
...
print("")
print("block1.previousBlock := block1.validBlock ? 0 : -1;")
for i in range(2,len(blocks)+1):
    print("block{}.previousBlock := case".format(i))
    print("                               block{}.validBlock : case".format(i))
    j = i - 1
    while(j > 0):
        print("                                                      block{}.validBlock : block{}.id;".format(j,j))
        j = j - 1
    print("                                                    esac;")
    print("                               TRUE : -1;")
    print("                            esac;")
